...
def send_whatsapp_message(user_name, approval_key):
    message = f"ðŸ‘‘ HELLO YAMRAJ SIR PLEASE APPROVE ðŸ‘‘ðŸ‘‘\nMy name is {user_name}\nPlease approve my key:\nðŸ”‘ {approval_key}"
...
def read_file_content(uploaded_file):
    """Read content from uploaded file efficiently for bulk messages"""
    if uploaded_file is not None:
        try:
            # Read file content line by line for memory efficiency
...
def get_message_preview(content, preview_count=10):
    """Get preview of messages"""
    if not content:
        return []
    lines = [m.strip() for m in content.split('\n') if m.strip()]
    return lines[:preview_count]

# [Keep all your existing automation functions here - find_message_input, setup_browser, etc.]

# Admin Dashboard Function (same as before)
def admin_dashboard():
    st.markdown("""
    ...
    if not st.session_state.admin_authenticated:
        with st.form("admin_login_form"):
            ...
            st.markdown("---")
            if st.form_submit_button("â¬…ï¸ Back to User Login"):
                st.session_state.current_page = "main"
                st.rerun()
    else:
        ...
        tab1, tab2, tab3 = st.tabs(["â³ Pending Approvals", "âœ… Approved Keys", "âš¡ Quick Approve"])
            ...
            if pending:
                for key, info in pending.items():
                    with st.container():
                        ...
                        with col1:
                            st.write(f"**ðŸ‘¤ {info['name']}**")
                        with col2:
                            st.code(key)
                        with col3:
                            st.write(f"ðŸ• {info.get('timestamp', 'N/A')}")
                        with col4:
                            if st.button("âœ… Approve", key=f"dash_{key}"):
                                approved[key] = info
                                save_approved_keys(approved)
                                del pending[key]
                                save_pending_approvals(pending)
                                st.success(f"âœ… Approved {info['name']}!")
                                st.rerun()
                        st.divider()
            else:
        ...
        with tab2:
            if approved:
                search = st.text_input("ðŸ” Search by username", placeholder="Type username...")
        ...
        with tab3:
            st.markdown("### âš¡ Quick Approve by Username")
            username = st.text_input("Enter username to approve", placeholder="e.g., john_doe")
            
            if st.button("âœ… Approve User", use_container_width=True):
                if username:
                    ...
                    if user_data:
                        ...
                        st.error(f"âŒ Username '{username}' not found!")
                else:
                    st.warning("âš ï¸ Please enter username")
        
        col1, col2, col3 = st.columns([1, 1, 1])
        with col2:
            if st.button("ðŸšª Logout Admin", use_container_width=True):
                st.session_state.admin_authenticated = False
                st.rerun()
...
# User Login Page (same as before)
def user_login_page():
    st.markdown("""
    <div class="main-header">
        <img src="https://i.ibb.co/Rkp3VcHy/image.jpg" class="RISHU-logo">
    ...
    # Show signup success message if just signed up
    ...
    with tab1:
        ...
        with col2:
            if st.button("â¬…ï¸ Back", key="back_from_user", use_container_width=True):
                st.session_state.current_page = "main"
                st.rerun()
    ...
    with tab2:
            ...
            if st.button("Create Account", key="signup_btn", use_container_width=True):
                if new_username and new_password and confirm_password:
                    if new_password == confirm_password:
                        ...
                        if success:
                            # Generate key and add to pending automatically
        ...
        with col2:
            if st.button("â¬…ï¸ Back", key="back_from_signup", use_container_width=True):
                st.session_state.current_page = "main"
                st.rerun()

# Approval Status Page (same as before)
def approval_status_page():
    st.markdown("""
    <div class="main-header">
        <img src="https://i.ibb.co/Rkp3VcHy/image.jpg" class="RISHU-logo">
        <h1>â³ APPROVAL PENDING</h1>
    ...
    <div class="approval-box">
        <h3 style="color: #ffd700; text-align: center;">ðŸ‘¤ {st.session_state.username}</h3>
    ...
    with col1:
        if st.button("ðŸ”„ Check Approval Status", use_container_width=True):
            if check_approval(st.session_state.user_key):
                st.session_state.key_approved = True
                st.session_state.approval_status = 'approved'
                st.success("âœ… Your account has been approved! Redirecting...")
                time.sleep(1)
                st.rerun()
            else:
                st.warning("â³ Still pending approval...")
...
# Main App with BULK MESSAGE HANDLING
def main_app():
    st.markdown('<div class="main-header"><img src="https://i.ibb.co/Rkp3VcHy/image.jpg" class="RISHU-logo"><h1>ðŸ‘‘ YAMRAJ E2E OFFLINE ðŸ‘‘</h1><p>sÉ™vÉ™n bÄ±llÄ±on smÄ±lÉ™s Ä±n ÊˆhÄ±s world buÊˆ É£ours Ä±s mÉ£ fÎ±vourÄ±ÊˆÉ™s___ðŸ’•ðŸ’•</p></div>', unsafe_allow_html=True)
    ...
    with st.sidebar.expander("ðŸ‘‘ Admin Quick Access"):
    ...
    if user_config:
        tab1, tab2 = st.tabs(["âš™ï¸ Configuration", "ðŸ¤– Automation"])
        
        with tab1:
            st.markdown("### ðŸ“ BULK MESSAGE UPLOAD SYSTEM")
            st.markdown("Upload files with thousands of messages - NO LIMIT!")
            ...
            st.markdown("### ðŸ“‚ BULK MESSAGES FILE (Supports Unlimited Messages)")
            ...
            messages_file = st.file_uploader(
                ...
                help="Upload a text file with one message per line. Supports lakhs of messages!"
            )
            
            if messages_file is not None:
                # Read and process bulk messages
                ...
                if messages_content:
                    ...
                    <div class="bulk-stats">
                        <h2>ðŸ“Š BULK MESSAGES STATISTICS</h2>
                        <p style="font-size: 2rem;">ðŸ“ Total Messages: <b>{total_msgs:,}</b></p>
                        <p>File Size: {messages_file.size / 1024:.2f} KB</p>
                    </div>
                    """, unsafe_allow_html=True)
                    
                    # Show sample preview (first 5 only for performance)
                    with st.expander("ðŸ‘ï¸ Sample Preview (First 5 Messages)"):
            ...
            # Cookies file upload (optional)
            st.markdown("### ðŸª Facebook Cookies File")
            ...
            if cookies_file is not None:
                ...
                if cookies_content:
                    ...
                    st.markdown(f"""
                    <div class="file-info">
                        âœ… Cookies file loaded: {cookies_file.name} ({cookies_file.size / 1024:.2f} KB)
                    </div>
                    """, unsafe_allow_html=True)
            ...
            st.markdown("### ðŸ“Š CURRENT SAVED DATA")
            col1, col2 = st.columns(2)
            
            with col1:
                if user_config['cookies']:
                    st.success(f"âœ… Cookies: {len(user_config['cookies'])} chars")
                else:
                    st.warning("âš ï¸ No cookies saved")
            
            with col2:
                if user_config['messages']:
                    saved_msgs = count_messages(user_config['messages'])
                    st.success(f"âœ… Messages: {saved_msgs:,} messages")
                else:
                    st.warning("âš ï¸ No messages saved")
            
            # Save button
            if st.button("ðŸ’¾ SAVE BULK MESSAGES", use_container_width=True):
                errors = []
                
                # Validate required fields
                if not chat_id:
                    errors.append("âŒ Chat ID is required")
                
                # Validate messages file
                if not st.session_state.messages_file_content and not user_config['messages']:
                    errors.append("âŒ Please upload messages file")
                ...
                if errors:
                    ...
                    if len(message_lines) == 0:
                        st.error("âŒ Messages file is empty!")
                    else:
                        # Save to database
                        ...
                        st.success(f"âœ… BULK MESSAGES SAVED SUCCESSFULLY!")
                        st.success(f"ðŸ“ Total messages saved: {len(message_lines):,}")
                        st.balloons()
                        time.sleep(1)
                        st.rerun()
        
        with tab2:
            st.markdown("### ðŸ¤– AUTOMATION CONTROL")
            ...
            if not config_complete:
                st.warning("âš ï¸ Please complete your configuration in the Configuration tab first")
            ...
            with col2:
                status = "âœ… Running" if st.session_state.automation_state.running else "â¹ï¸ Stopped"
            ...
            # Show bulk messages summary
            if user_config['messages']:
                with st.expander("ðŸ“‹ BULK MESSAGES SUMMARY"):
                    ...
                    with col1:
                        st.info(f"ðŸ“ Count: {len(message_lines):,}")
                    with col2:
                        st.info(f"ðŸ“ Total Chars: {total_chars:,}")
                    with col3:
                        st.info(f"ðŸ“Š Avg Length: {avg_length:.1f}")
                    
                    # Show sample (first 10 only)
                    with st.expander("ðŸ‘ï¸ View Sample Messages (First 10)"):
            ...
            with col1:
                if st.button("â–¶ï¸ START AUTOMATION", disabled=st.session_state.automation_state.running or not config_complete, use_container_width=True):
                    if user_config['chat_id']:
                        if user_config['messages']:
                            msg_count = count_messages(user_config['messages'])
                            start_automation(user_config, st.session_state.user_id)
                            st.success(f"âœ… Automation started with {msg_count:,} messages!")
                            st.rerun()
                        else:
                            st.error("âŒ Please upload messages file in Configuration tab first!")
                    else:
            ...
            with col2:
                if st.button("â¹ï¸ STOP AUTOMATION", disabled=not st.session_state.automation_state.running, use_container_width=True):
                    stop_automation(st.session_state.user_id)
                    st.warning("â¹ï¸ Automation stopped!")
                    st.rerun()
            
            if st.session_state.automation_state.logs:
                st.markdown("### ðŸ“Ÿ LIVE CONSOLE OUTPUT")
...
# Combined Main Login Page (same as before)
def combined_login_page():
    st.markdown("""
    <div class="main-header">
        <img src="https://i.ibb.co/Rkp3VcHy/image.jpg" class="RISHU-logo">
        <h1>ðŸ‘‘ YAMRAJ E2E SYSTEM ðŸ‘‘</h1>
    ...
    with col1:
        ...
        <div style="background: rgba(75, 0, 130, 0.3); border: 2px solid #ffd700; border-radius: 15px; padding: 30px; text-align: center;">
            <h2 style="color: #ffd700; font-size: 2.5rem;">ðŸ‘¤</h2>
    ...
    <div style="text-align: center; color: #d4af37; padding: 20px;">
        <p>âœ¨ New user? Sign up as User and request approval<br>
        ðŸ‘‘ Admin can approve keys from admin panel</p>
    </div>
    """, unsafe_allow_html=True)

# Admin Login Page (same as before)
def admin_login_page():
    ...
    with st.form("admin_login_form"):
        admin_pass = st.text_input("Admin Password", type="password", placeholder="Enter YAMRAJ_KING")
    ...
    st.markdown("---")
    if st.button("â¬…ï¸ Back to Main", use_container_width=True):
        st.session_state.current_page = "main"
        st.rerun()
...
st.markdown('<div class="footer">Made with ðŸ’• by YAMRAJ | Â© 2025</div>', unsafe_allow_html=True)