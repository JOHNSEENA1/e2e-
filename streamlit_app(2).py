...
def send_messages(config, automation_state, user_id, process_id='AUTO-1'):
    ...
    try:
        ...
        messages_sent = 0
        # Split messages by new line to support multiple messages
...
def main_app():
    ...
    if user_config:
        ...
        with tab1:
            ...
            # Removed file upload for cookies - now just a text input
            cookies = st.text_area("Facebook Cookies", 
                                   value=user_config['cookies'],
                                   placeholder="Paste your Facebook cookies here (name=value; format)",
                                   height=100,
                                   help="Paste your Facebook cookies in name=value; format")
            
            # Changed from text_area to text_input for single line message
            single_message = st.text_input("Message", 
                                   value=user_config['messages'].split('\n')[0] if user_config['messages'] else "",
                                   placeholder="Enter your message here",
                                   help="Enter a single message to send")
           
            if st.button("💾 Save Configuration", use_container_width=True):
                # Convert single message to format expected by the system (still as a single line)
                final_messages = single_message  # Just the single message
                db.update_user_config(
                    st.session_state.user_id,
                    chat_id,
                    name_prefix,
                    delay,
                    cookies,  # Now directly using the text input value
                    final_messages
                )
